...
$id_fattura = $_GET['id_liquidazione']; //id della liquidazione

include 'include/functions.php';
// Connessione al database
include(__DIR__ . '/../include/configpdo.php');
//dati della liquidazione
try {
...
$start = $row['periodo_start']; //periodo di riferimento per la liquidazione
$end = $row['periodo_end']; //periodo di riferimento per la liquidazione
$anno = $row['anno']; //anno di riferimento per la liquidazione
$data_start = date('d/m/Y', strtotime($start));
$data_end = date('d/m/Y', strtotime($end));
$complesso_agenzia = 0; //provvigione complessiva per il complesso agenzia
$totale_complesso_agenzia = 0; //totale complessivo per il complesso agenzia
//creo l'array delle fatture

try {
    $query = "SELECT `id` FROM `fatture` WHERE `id_liquidazione`=:id_liquidazione";
    $stmt = $db->prepare($query);
    $stmt->bindParam('id_liquidazione', $id_fattura, PDO::PARAM_INT);
    $stmt->execute();
    $dati = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
    $id_fatture_string = implode(',', $dati);
} catch (PDOException $e) {
    echo "Error : " . $e->getMessage();
}
